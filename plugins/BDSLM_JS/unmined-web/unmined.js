...
class Unmined {
    ...
    constructor(mapElement, options, regions) {
        ...
        this.options = { ...Unmined.defaultOptions, ...options };  // 改为公共属性
        ...
        const map = new ol.Map({
            ...
            view: new ol.View({
                center: ol.proj.transform([this.options.centerX, this.options.centerZ], this.dataProjection, this.viewProjection), // 中心[0, 0],
                extent: mapExtent,
                projection: this.viewProjection,
                resolutions: tileGrid.getResolutions(),
                maxZoom: mapZoomLevels, //1-(-6) = 7
                zoom: mapZoomLevels - this.options.maxZoom,  //7 - 1 = 6
        ...
        for (var i = 0; i < markers.length; i++) {
            ...
            if (item.image)
                style.setImage(new ol.style.Icon({
                    ...
                    scale: bdsLMResolvedMarkerIconScale(item)
                }));

            if (item.text) {
                style.setText(new ol.style.Text({
                    text: item.text,
                    font: item.font || "12px Microsoft Yahei",
    ...
    createContextMenu() {
        ...
        contextmenu.on('open', (evt) => {
            ...
            contextmenu.push({
                text: `在此处放置红点标记`, // Place red dot marker here
            ...
            if (this.redDotMarker.getCoordinates()) {
                contextmenu.push({
                    text: `复制标记链接`,  //Copy marker link
                    callback: () => {
                        Unmined.copyToClipboard(window.location.href);
                    }
                });
                contextmenu.push({
                    text: `清除标记`, //Clear marker
                    callback: () => {
...
let unmined = new Unmined('map', UnminedMapProperties, UnminedRegions);